...
static void svg_paintable_init_interface (GdkPaintableInterface *iface);

G_DEFINE_TYPE_WITH_CODE (SvgPaintable,
			 svg_paintable, G_TYPE_OBJECT,
                        ...
                        double        height)
{
	...
	if (!rsvg_handle_render_document (self->handle, cr,
					  ...
					  &error)) {
		g_warning_once ("%s", error->message);
	}
